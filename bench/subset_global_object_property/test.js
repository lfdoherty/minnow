...
function run(){
	...
	minnow.makeServer(u.config, function(){
		...
		minnow.makeClient(u.config.port, function(client){
			client.view('empty', function(err, v){
				if(err) throw err
				
				var settings = v.make('settings', {minAge: 18}, function(){
				
					client.view('specific', [settings], testAgeThreshold(minnow, u.config.port, function(){
