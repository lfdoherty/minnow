...
exports.makeSync = function(s, rel, recurse, handle, ws, staticBindings){
	...
	if(maps.length === 0){
		...
		_.errout('why each-optimize if there are no maps?')
	}
	...
	//console.log('external bindings: ' + JSON.stringify(externalBindingsUsed))
	//console.log(JSON.stringify(rel, null, 2))
	
	externalBindingsUsed.forEach(function(bk){
		if(staticBindings[bk] === undefined) _.errout('missing binding ' + bk + ': ' + JSON.stringify(Object.keys(staticBindings)))
		_.assertDefined(staticBindings[bk].getStateAt)
	})
	...
	var newHandle = {
		...
		getAt: function(bindings, editId){
			...
			var state = inputSet.getAt(bindings, editId)
			if(state.length === 0){
				return []
			}
			...
			var bindingValues = {}
			externalBindingsUsed.forEach(function(bk){
				bindingValues[bk] = staticBindings[bk].getAt(bindings, editId)
			})
			var mapStates = []
			mapHandles.forEach(function(mh,i){
				mapStates[i] = mh.handle.getAt(bindings, editId)
			})
		...
		getBetween: function(bindings, startEditId, endEditId){
			...
			externalBindingsUsed.forEach(function(bk){
				changes = staticBindings[bk].getChangesBetween(bindings, startEditId, endEditId)
			...
			function computeIncrementalOneToOne(){
				...
				var changes = inputSet.getBetween(bindings, startEditId, endEditId)
				...
				externalBindingsUsed.forEach(function(bk){
					oldBindingValues[bk] = staticBindings[bk].getAt(bindings, startEditId)
					bindingValues[bk] = staticBindings[bk].getAt(bindings, endEditId)
				})
				...
				var mapChanges = []
				mapHandles.forEach(function(mh,i){
					mapChanges[i] = mh.handle.getBetween(bindings, startEditId, endEditId)
				})
				...
				for(var i=0;i<mapHandles.length;++i){
					...
					for(var j=0;j<mcc.length;++j){
						var c = mcc[j]
						if(c.type === 'put'){
							if(keyedChanges[c.key] === undefined){
								keyedChanges[c.key] = {}
								changedKeys.push(c.key)
							}
							keyedChanges[c.key][mh.uid] = c.value
				...
				mapHandles.forEach(function(mh,i){
					...
					oldMapStates[i] = mh.handle.getPartialAt(bindings, startEditId, needOldStateKeys)
					mapStates[i] = mh.handle.getPartialAt(bindings, endEditId, changedKeys)
				})
				...
				changeTransactions.forEach(function(c){
					...
					if(c.input){
						...
						mapHandles.forEach(function(mh,i){
							var cc = c['mh'+i]
							if(cc){
								_.assertDefined(cc.key)
								changingId = cc.key
								editId = cc.editId
							}
						})
						/*mapHandles.forEach(function(mh, i){
							if(!mh.handle.getValueStateAt) _.errout('missing getValueStateAt: ' + mh.handle.name)
							mh.handle.getValueStateAt(bindings, editId-1, changingId, function(pv){
					...
					mapHandles.forEach(function(mh, i){
						if(!mh.handle.getValueStateAt) _.errout('missing getValueStateAt: ' + mh.handle.name)
						mh.handle.getValueStateAt(bindings, editId, changingId, function(pv){
