...
exports.load = function(app, appName, schema, identifier, viewSecuritySettings, minnowClient, syncHandleCreationListener){
	...
	app.get(exports, '/mnw/sync/'+appName+'/:random', identifier, function(req, httpRes){
		...
		minnowClient.beginSync(listenerCb, objectCb, makeCb, function(syncId, syncHandle){
			theSyncId = syncId
			syncHandle.owningUserId = req.userToken
			syncHandles[syncId] = syncHandle
			log('got sync handle: ' + syncId)
			if(syncHandleCreationListener) syncHandleCreationListener(req, syncId)
	...
	app.post(exports,  '/mnw/xhr/update/' + appName + '/:syncId', identifier, function(req, res){
		...
		if(syncHandle.owningUserId !== req.userToken){
			...
			console.log('WARNING: user(' + req.userToken + ') attempted to access sync handle of user(' + syncHandle.owningUser + ') - access denied')
			res.send(403)
			return
		}
