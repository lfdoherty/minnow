...
W.prototype.writeBuffer = function(nextSize, cb){	
	...
	if(this.position > 0){
		...
		//console.log('writingBuffer: ' + this.position + ' ' + this.b.length)
		//console.log(new Error().stack)
		//if(this.b.length - this.position > this.position){
		//	this.b = this.b.slice(this.position)
		//}else{
...
W.prototype.putData = function(buf, off, end){
	_.assertLength(arguments, 3)
	//if(length === undefined) length = buf.length;
	var len = end-off
	this.prepareFor(len);
	buf.copy(this.b, this.position, off, end);
	this.position += len
}
